...
Particle_Src<T>::Particle_Src(int _part_type,float _x,float _y,std::list <Phys_Object *> &_objs,int size_x,int size_y,int nbr_total_part,unsigned int nbr_part_now,float _speed_m,float _speed_s,float _angle_m,float _angle_s,float _density_m,float _density_s)
	: Phys_Object(size_x,size_y),lock(NULL),lock_x(0),lock_y(0),part_type(_part_type),particules_remaining(nbr_total_part-nbr_part_now),speed_m(_speed_m),speed_s(_speed_s),
	angle_m(_angle_m),angle_s(_angle_s),density_m(_density_m),density_s(_density_s),nbr_part_to_throw(0.0),
	is_finished(false),objs(_objs)
{
...
bool Particle_Src<T>::update()
{
	...
	while(nbr_part_to_throw>1)
	{
		...
		objs.push_back(obj);
		nbr_part_to_throw--;
		if (particules_remaining>0)
		{
			particules_remaining--;
			if (particules_remaining==0) return true;
		}
	}
